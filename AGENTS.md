...
node scripts/test_edit_assist.mjs   # ranking/asignación determinista del montaje asistido
node scripts/test_edit_asset_relink.mjs   # matching determinista del manifiesto de medios
node scripts/e2e-edit-studio-assist.mjs   # borrador reversible Actual/Borrador y aplicación (requiere server :3210)
node scripts/e2e-edit-studio-relink.mjs   # reload + reconexión masiva y por carpeta sin cambiar IDs; quitar pendientes desbloquea (requiere server :3210)
node scripts/e2e-edit-studio-mixed.mjs   # e2e mixto: cortes internos detectados, orden narrativo, export Tinta/Viñetas/Profundidad/Whip sin frames negros (requiere server :3210)
...
Montaje multiclip vertical al beat para Shorts (9:16 fijo, 1080×1920). Workspace de escritorio a alto completo: preview y timeline con thumbnails permanecen visibles; solo el inspector por pestañas (`Montaje|Medios|Toma|Acabado`) hace scroll. En móvil el preview es sticky. Hay 11 transiciones + 11 movimientos por toma + velocidad con curva. Librerías:
- `src/lib/editStudio.ts` — `EditProject` (9:16 fijo, `fps 30|60`, BPM 128 default), `EditTimelineClip` con `transition` (`cut|punch|shake|zoom|flash|whip|crossfade|blur|depth|ink|panel`), fuerza/dirección de transición (`transitionIntensity`, `transitionDirection`), `motion` (`static|push|pull|drift|whip|vertigo|spiral|scan|impact|parallax|parallaxDrift`), encuadre por toma (`framingX/Y/Scale`), `motionIntensity` 0..100, `playbackRate` 0.5..2.0 real y `velocityCurve` (`linear|easeIn|easeOut|punch`); `normalizeEditClip` migra proyectos v1 sin cambiar la versión.
- `src/lib/editStudioRender.ts` — preview y export comparten la composición de las 11 transiciones; `depth`, `ink` y `panel` simulan profundidad/capas manga sin red ni modelo, y `parallax*` separa fondo, foco y primer plano con canvas. `drawEditTextCue` conserva cuatro estilos (`impact|condensed|editorial|minimal`), énfasis, multilínea con `|` y autoajuste al ancho seguro.
- `src/lib/editPresets.ts` — seis montajes dirigidos basados en las referencias: Impacto progresivo (~18 s), Emoción cinematográfica (~18,3 s), Collage gráfico (~13 s), **Pulso ascendente** (~26,8 s, The Climber), Tráiler lírico (30 s) y Retrato hipnótico (~9,8 s). Los presets no inventan texto: conservan únicamente cues ya escritos por el usuario.
- `src/lib/editAssistAnalysis.ts` — análisis local en dos pasadas: hasta 160 muestras gruesas a 72 px para detectar escenas y hasta 48 descriptores a 160 px para calidad/energía/movimiento/saliencia/dHash; la canción usa ventanas de 50 ms. Es cancelable, cacheado durante la sesión y no usa red/modelos ni conserva luminancia después de analizar.
- `src/lib/editAssistPlanner.ts` — elige estructura/duración útil (≈10–32 s) y ventana musical, después asigna medios o escenas mediante beam search (ancho 32): evita duplicados consecutivos, penaliza retrocesos narrativos de forma blanda y nunca cruza un corte interno detectado. `EditAssistDraft` incluye hasta tres SFX discretos sugeridos y reversibles.
- `src/lib/editProjectAssets.ts` — manifiesto persistente y matching determinista por nombre/basename/metadatos para reconectar varios medios en una sola selección conservando `assetId` y timeline.
...
Particularidades: `ClipFrameCache` (540px, 18fps, 88MB) para preview RAF velocity-aware (`editSourceTimeAt`); export re-decoodifica a 1080×1920 nativo. El montaje asistido nunca muta ni persiste `EditProject`/SFX durante análisis o revisión: Actual muestra timeline e inspector en solo lectura; Borrador muestra y permite editar proyecto + SFX; solo `Aplicar montaje` persiste ambos. `.loop-edit.json` y localStorage guardan el manifiesto de medios; tras reload los `File` se reconectan por lote sin crear clips nuevos — la sección Medios acepta archivos sueltos o una carpeta completa (filtro de medio editable, `webkitdirectory`) y cada medio pendiente se puede quitar con × (borra sus tomas y libera el bloqueo de export). La restauración ocurre después de hidratar para que SSR y cliente no diverjan.
