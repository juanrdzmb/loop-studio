...
## Continuidad visual automática (LoopyCut)

- `companion /analyze/video` devuelve candidatos con contrato `kind`, `fade_sec`, `reason`; el clip completo ya no recibe score 100 artificial. `pickVisualLoop` favorece una costura detectada con score ≥70 y cobertura ≥50%; si no existe, conserva el clip entero con fundido conservador.
- `recommendVisualLoopForClip` se usa en 16:9 y rechaza análisis cuya duración no coincide con los metadatos del navegador. **9:16 siempre usa el clip completo** (`sourceStart=0`, `sourceEnd` omitido), sin recortes LoopyCut; preview, seek y export comparten ese rango. El modo automático siempre es `smooth`; `pingpong` es manual.
...
- Test de contrato/ranking: `companion/.venv/bin/python scripts/test_companion_video_loop.py`. E2E de integración y exclusión de frames fuera del rango: `node scripts/e2e-visual-loop-auto.mjs` (requiere companion en :8787).
...
- **Ajustes avanzados de slowed+reverb plegados**: los 6 sliders viven en un `<details>`; los presets quedan visibles.
...
- **Upload = canvas limpio**: al subir un video, todos los efectos (estilo, partículas, modo de costura, cámara) se resetean a su valor neutro: `original`, `none`, `smooth`, `static`. El usuario decide qué añadir después. Los handlers `handleVideo16x9` y `handleVideo9x16` aplican este reset explícitamente; los `useState` iniciales también arrancan limpios.
